...
export default async function MintProfilePage({ params }: { params: Promise<{ username: string }> }) {
  ...
  const [session, t] = await Promise.all([auth(), getTranslations('mint')])
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start', marginBottom: 28, flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 8, flexWrap: 'wrap' }}>
            {user.isEstablished && <span style={{ fontSize: 11, background: '#1e2a15', color: 'var(--green)', fontWeight: 700, padding: '2px 8px', borderRadius: 4 }}>{t('establishedBadge')}</span>}
            {user._count.createdItems > 0 && <span style={{ fontSize: 11, background: '#1e1a0a', color: 'var(--gold)', fontWeight: 700, padding: '2px 8px', borderRadius: 4 }}>{t('creatorBadge')}</span>}
            {isMutual && <span style={{ fontSize: 11, background: 'rgba(100,160,220,0.12)', color: '#7ab8e8', fontWeight: 700, padding: '2px 8px', borderRadius: 4 }}>{t('mutualBadge')}</span>}
          </div>

          {/* Follow counts */}
          <div style={{ display: 'flex', gap: 16, marginTop: 10 }}>
            <span style={{ fontSize: 13, color: 'var(--muted)' }}>
              <span style={{ fontWeight: 700, color: 'var(--white)' }}>{followerCount.toLocaleString()}</span> {t('followers')}
            </span>
            <span style={{ fontSize: 13, color: 'var(--muted)' }}>
              <span style={{ fontWeight: 700, color: 'var(--white)' }}>{followingCount.toLocaleString()}</span> {t('following')}
            </span>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'flex-end' }}>
          {isOwn ? (
            <Link href="/settings" className="btn btn-outline btn-sm">{t('editMint')}</Link>
          ) : (
            <FollowButton
      ...
      <div className="stats-row" style={{ marginBottom: jobDef ? 16 : 32 }}>
        ...
        {totalMonthlyIncome > 0 ? (
          <div className="stat-box">
            <div className="stat-label">{t('statMonthlyIncome')}</div>
            <div className="stat-value" style={{ color: 'var(--green)' }}>+${totalMonthlyIncome.toLocaleString()}</div>
          </div>
        ) : (
          <div className="stat-box">
            <div className="stat-label">{t('statItems')}</div>
      ...
      {jobDef && user.job && (
        <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 10, padding: '14px 18px', marginBottom: 28, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10 }}>
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--muted)', letterSpacing: '0.08em', marginBottom: 2 }}>{t('employed')}</div>
          ...
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--muted)', letterSpacing: '0.06em' }}>{t('monthlySalary')}</div>
            ...
            <div key={cat} style={{ marginBottom: 32 }}>
              ...
              <div className="items-grid">
                {editions.map(e => {
                  ...
                  if (bNet > 0) {
                    subLine = { text: t('incomePerMonth', { amount: bNet.toLocaleString() }), colour: 'var(--green)' }
                  } else if (e.item.propertyTier) {
                    subLine = pUpkeep > 0
                      ? { text: t('upkeepPerMonth', { amount: pUpkeep.toLocaleString() }), colour: 'var(--red)' }
                      : { text: t('noUpkeep'), colour: 'var(--muted)' }
                  } else if (aUpkeep > 0) {
                    subLine = { text: t('upkeepPerMonth', { amount: aUpkeep.toLocaleString() }), colour: 'var(--red)' }
                  } else if (carUpkeep > 0) {
                    subLine = { text: t('upkeepPerMonth', { amount: carUpkeep.toLocaleString() }), colour: 'var(--red)' }
                  } else {
                    ...
                    <Link key={e.id} href={`/item/${e.id}`} style={{ textDecoration: 'none' }}>
                      <div className={`item-card tier-${e.item.rarityTier.toLowerCase()}`} style={{ position: 'relative' }}>
                        {isOwn && e._count.offers > 0 && (() => { badgeTop = 6; return (
                          <span style={{ position: 'absolute', top: badgeTop, right: 6, background: 'var(--gold)', color: '#000', fontSize: 10, fontWeight: 800, padding: '2px 6px', borderRadius: 4, zIndex: 2 }}>{t('offerBadge')}</span>
                        ) })()}
                        {e.isListed && (() => { const top = isOwn && e._count.offers > 0 ? 26 : 6; return (
                          <span style={{ position: 'absolute', top, right: 6, background: '#1a3a10', color: 'var(--green)', fontSize: 9, fontWeight: 800, padding: '2px 5px', borderRadius: 4, zIndex: 2 }}>{t('listedBadge')}</span>
                        ) })()}
