...
export default function CronPage() {
  ...
  const [result, setResult]   = useState<{ log: string[]; ok: boolean; upkeepCharged?: number; salaryPaid?: number; incomesPaid?: number; created?: number } | null>(null)
      ...
      {result && (
        <div style={{ marginTop: 28 }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 24 }}>
            {[
              { label: 'Auctions Created',  value: result.created        ?? 0 },
              { label: 'Upkeep Charged',    value: result.upkeepCharged  ?? 0 },
              { label: 'Salaries Paid',     value: result.salaryPaid     ?? 0 },
              { label: 'Business Incomes',  value: result.incomesPaid    ?? 0 },
            ].map(s => (
              <div key={s.label} style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 8, padding: '14px 16px', textAlign: 'center' }}>
                <div style={{ fontSize: 26, fontWeight: 900, color: 'var(--gold)' }}>{s.value}</div>
