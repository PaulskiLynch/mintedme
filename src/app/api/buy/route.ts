...
export async function POST(req: NextRequest) {
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      // Primary sale: use reference price; secondary: must be listed
      if (!isPrimarySale && (!edition.isListed || !edition.listedPrice)) {
        throw new Error('Item is not listed for sale')
      }
      if (isPrimarySale && !edition.item.referencePrice) {
        throw new Error('No price set for this item')
      }

      const price = isPrimarySale
        ? Number(edition.item.referencePrice)
        : Number(edition.listedPrice)
      ...
      // Creator cut: 80% on primary sale, 20% on secondary
      const creatorPct = creatorId && (isPrimarySale || creatorId !== sellerId)
        ? (isPrimarySale ? 0.8 : 0.2)
        : 0
      ...
      const txn = await tx.transaction.create({
        data: {
          ...
          description: `${isPrimarySale ? 'Bought' : 'Bought'} ${edition.item.name} #${edition.editionNumber}`,
        },
      })
      ...
      await tx.feedEvent.create({
        data: { eventType: 'buy', userId: buyerId, targetUserId: sellerId ?? buyerId, editionId, amount: price },
      })
